...
LastEditTime: 2024-04-02 00:38:33
...
class Network(nn.Module):
    def __init__(
        ...
        train_folder_name = define_name(name, lr, epoch, optimizer, use_sche, "train")
        # Training save dirs
        ...
        # Inference save dirs
        test_folder_name = define_name(name, lr, epoch, optimizer, use_sche, "test")
    ...
    def train(self) -> None:
        for epoch in range(self.start_epoch, self.max_epoch):
            ...
            with tqdm(self.tr_loader, unit="batch") as tepoch:
                for batch in tepoch:
                    tepoch.set_description(f"Epoch {epoch+1}/{self.max_epoch} Training")
                ...
                with torch.no_grad():
                    with tqdm(self.val_loader, unit="batch") as tepoch:
                        for batch in tepoch:
                            ...
                            y[y != 0] = 1  # convert to binary mask
                            y_pred = self.model(x)
                            y_pred_logits = torch.argmax(
                                torch.softmax(y_pred, dim=1), dim=1, keepdim=True
                            )
        ...
        with torch.no_grad():
            with tqdm(self.te_loader, unit="batch") as tepoch:
                for batch in tepoch:
                    ...
                    y_pred_label_numpy = (
                        y_pred_label.detach().cpu().numpy().astype(np.uint8)[0, 0, ...]
                    )
                    y_pred_label_numpy[y_pred_label_numpy != 0] = 255
                    seg = Image.fromarray(y_pred_label_numpy)
                    seg.save(os.path.join(self.inference_dir, filename))
                    y_pred_class = monai.networks.utils.one_hot(
                    ...
                    list_dsc = (
                        dice_score.detach().cpu().numpy().astype(np.float64).tolist()
                    )
                    list_iou = (
                        iou_score.detach().cpu().numpy().astype(np.float64).tolist()
                    )
                    dscs.append(list_dsc)
                    ious.append(list_iou)
                    results[filename] = {"Dice Score": list_dsc, "IoU": list_iou}

        avg_dsc = np.mean(dscs, axis=0)
        avg_iou = np.mean(ious, axis=0)
        results["Average"] = {"Dice Score": avg_dsc.tolist(), "IoU": avg_iou.tolist()}
        with open(os.path.join(self.inference_dir, "results.json"), "w") as f:
    ...
    def full_score(
        ...
        return self.full_dice_metric(y_pred, y)[0].flatten(), 1 - (
            self.full_jaccard_loss(y_pred, y).flatten()
        )
    ...
    def load_checkpoint(
        ...
        assert os.path.exists(
            os.path.join(self.weights_dir, f"{mode}_ckpt.pth")
        ), f"{mode.capitalize()} checkpoint not found !!!"
        ckpt = torch.load(
            os.path.join(self.weights_dir, f"{mode}_ckpt.pth"), map_location=self.device
        )
    ...
    def save_checkpoint(self, mode: str = None) -> None:
    ...
    def plot_architecture(self, mode: str) -> None:
        ...
        make_dot(
            ...
            show_attrs=True,
            show_saved=True,
        ).render(filename="network", directory=save_dir, format="png")
